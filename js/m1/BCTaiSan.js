...
class BCTaiSan extends Component {
 ...
 constructor(props) {
  ...
  getReportData(reportType, month, year) {
    ...
    let url = MyConst.WS_URL + 'report/bcts?type=' + reportType + '&month=' + month + "&year=" + year;
    fetch(url)
      .then(res => res.json())
      .then(responseJson => {
        this.setState({
                    ...
                    <FlatList
                        ...
                        ListHeaderComponent={() => 
                                    <ListItem style={styles.liHeader}>
                                      <Text style={[styles.liTextHeader, {width:"30%"}]}>Tài sản</Text>
                                      <Text style={[styles.liTextHeader, {width:"25%", textAlign:"right"}]}>Nguyên giá</Text>
                                      <Text style={[styles.liTextHeader, {width:"20%", textAlign:"right"}]}>Khấu hao</Text>
                                      <Text style={[styles.liTextHeader, {width:"25%", textAlign:"right"}]}>GT còn lại</Text>
                                    </ListItem>}
  ...
  _renderListItem({item, index}) {
    ...
    if(index % 2 == 0)  {
      return  <ListItem style={styles.liEven}>
                <Text style={[styles.liText, {width:"30%"}]}>{item.name}</Text>                
               
                <TextMask	style={[styles.liText, {width:"25%", textAlign:"right"}]} value={item.grossBookValue}	type={'money'}	options={{unit: '', precision: 0}} />
                <TextMask	style={[styles.liText, {width:"20%", textAlign:"right"}]} value={item.depreValue}	type={'money'}	options={{unit: '', precision: 0}} />
                <TextMask	style={[styles.liText, {width:"25%", textAlign:"right"}]} value={item.remainValue}	type={'money'}	options={{unit: '', precision: 0}} />
              </ListItem> 
    } else {
