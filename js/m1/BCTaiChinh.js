...
class BCTaiChinh extends Component {
 ...
 constructor(props) {
  ...
  getReportData(reportType, month, year) {
    ...
    let url = MyConst.WS_URL + 'report/bctc?type=' + reportType + '&month=' + month + "&year=" + year;
    fetch(url)
      .then(res => res.json())
      .then(responseJson => {
        this.setState({
                    ...
                    <FlatList
                        ...
                        ListHeaderComponent={() => 
                                    <ListItem style={styles.liHeader}>
                                      <Text style={[styles.liTextHeader, {width:"60%"}]}>Nội dung</Text>
                                      <Text style={[styles.liTextHeader, {width:"40%", textAlign:"right"}]}>Giá trị</Text>
                                    </ListItem>}
